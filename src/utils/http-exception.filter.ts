...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    const errorResponse = {
      ...
      timestamp: `${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`, // 错误的时间
    };
    // 打印日志
    Logger.error(
      `【${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}】${request.method} ${request.url}`,
